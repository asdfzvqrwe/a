...
class BluetoothChatService(context: Context?, handler: Handler) {
    ...
    private fun updateUserInterfaceTitle() {
        ...
        mHandler.obtainMessage(Constants.Companion.MESSAGE_STATE_CHANGE, mNewState, -1).sendToTarget()
    }
    ...
    fun connected(socket: BluetoothSocket?, device: BluetoothDevice, socketType: String) {
        ...
        val msg = mHandler.obtainMessage(Constants.Companion.MESSAGE_DEVICE_NAME)
        val bundle = Bundle()
        bundle.putString(Constants.Companion.DEVICE_NAME, device.name)
        msg.data = bundle
    ...
    private fun connectionFailed() {
        ...
        val msg = mHandler.obtainMessage(Constants.Companion.MESSAGE_TOAST)
        val bundle = Bundle()
        bundle.putString(Constants.Companion.TOAST, "Unable to connect device")
        msg.data = bundle
        mHandler.sendMessage(msg)
        mState = STATE_NONE
        // Update UI title
    ...
    private fun connectionLost() {
        ...
        val msg = mHandler.obtainMessage(Constants.Companion.MESSAGE_TOAST)
        val bundle = Bundle()
        bundle.putString(Constants.Companion.TOAST, "Device connection was lost")
        msg.data = bundle
        mHandler.sendMessage(msg)
        mState = STATE_NONE
        // Update UI title
    ...
    private inner class ConnectedThread(socket: BluetoothSocket?, socketType: String) : Thread() {
        ...
        override fun run() {
            ...
            while (mState == STATE_CONNECTED) {
                try {
                    ...
                    mHandler.obtainMessage(Constants.Companion.MESSAGE_READ, bytes, -1, buffer)
                            .sendToTarget()
                } catch (e: IOException) {
        ...
        fun write(buffer: ByteArray?) {
            try {
                ...
                mHandler.obtainMessage(Constants.Companion.MESSAGE_WRITE, -1, -1, buffer)
                        .sendToTarget()
            } catch (e: IOException) {
